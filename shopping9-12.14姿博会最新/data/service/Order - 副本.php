...
class Order extends BaseService implements IOrder
{
     ...
     * @param unknown $create_time                      
     */
    public function updateCollectMoney($collect_id, $order_id, $goods_id, $goods_name, $reserve_phone, $order_no, $uid, $nick_name, $meet_money, $coupon_name, $coupon_id, $coupon_money, $at_leasts, $realPayment_money, $payment_method, $accountsWx_money, $paymentDate, $remark, $create_time)
    {
        $CollectMoney = new CollectMoney();
        ...
        if(!empty($collect_id)){
            ...
            if(empty($coupon_id)){
                ...
                unset($data['coupon_money']);
                unset($data['at_leasts']);
            }
            $collect = $CollectMoney->getInfo(['order_no'=>$order_no],'collect_id,coupon_id');
            $collect2 = $CollectMoney->getInfo(['collect_id'=>$collect_id],'order_no');
            if(!empty($collect)){
                return 2;
            }
            ...
            $result2 = true;
            if(!empty($collect['coupon_id'])&&$coupon_id!=$collect['coupon_id']){
                $row['state'] = 1;
                $row['use_time'] = 0;
                $result2 = $coupon->save($row,['coupon_id'=>$collect['coupon_id']]);
            }
            $result3 = true;
            if(!empty($coupon_id)&&$coupon_id!=$collect['coupon_id']){
                $row['state'] = 2;
                $row['use_time'] = time();
                $result3 = $coupon->save($row,['coupon_id'=>$coupon_id]);
            }
            $result4 = true;
            $result5 = true;
            if(!empty($order_no)&&$collect2['order_no']!=$order_no){
                $order_count = $order_model->getCount(['order_no'=>$order_no,'order_status'=>10]);
                if($order_count>0){
                   $orderArr['pay_status'] = 10;
                   $result4 = $order_model->save($orderArr,['order_no'=>$order_no]);  
                }
                $order_count2 = $order_model->getCount(['order_no'=>$collect2['order_no'],'order_status'=>10]);
                if($order_count2>0){
                   $orderArr2['pay_status'] = 0;
                   $result5 = $order_model->save($orderArr2,['order_no'=>$collect2['order_no']]);  
                }
            }
            ...
            if(!empty($orderId)){
                ...
                $orderArr['order_money'] = $meet_money;
            ...
            if(!empty($orderId)){
                ...
                $goodOrderArr['goods_money'] = $meet_money;
                $goodOrderArr['order_status'] = 10;
            ...
            if(!empty($orderId['order_id'])){
                ...
                foreach($do as $val){
                    $res['price'] = $realPayment_money;
                    $res['profit'] = $realPayment_money;
                    $res['commission_money'] = $res['profit']*($val['distribution_commission_rate']/100)*($val['commission_rate']/100);
                    $res['is_issue'] = 1;
     ...
     */
    public function orderCreateReservation($out_trade_no, $goods_sku_list, $use_coupon, $shipping_time, $num, $receiver_mobile, $seller_memo)
    {
        $order = new OrderBusiness();
        $retval = $order->orderCreateReservation($out_trade_no, $goods_sku_list, $use_coupon, $shipping_time, $num, $receiver_mobile, $seller_memo);
        runhook("Notify", "orderCreate", array(
            "order_id" => $retval
        ));
        // 针对特殊订单执行支付处理
            ...
            if(!empty($orderList)){
                foreach($orderList as $k=>$v){
                    ...
                    if($goodsCon['is_open']==1){
                        ...
                        //获取上级推广员和分销级别
                        $userCon = $user->getInfo(['uid'=>$v['buyer_id']],'Pid,level_id');
                        ...
                        if(!empty($relationList)){
                            foreach($relationList as $v){
                                if(LEVEL>$j){
                                    ...
                                    $data[$i] = $data[$i-1];
                                    $data[$i]['add_time'] = time();
                                    $data[$i]['receiptor_id'] = $v['Pid'];
                                    $data[$i]['commission_rate'] = $RromotersCon['level_'.$j];;
                                    $orderCommission += $data[$i]['commission_money'] = $data[$i]['profit']*($data[$i]['distribution_commission_rate']/100)*($RromotersCon['level_'.$j]/100);
                                    $j++;
                                }
                            }
                        }

...
class Order extends BaseController
{
    ...
    public function getOrderNo_user($order_no='')
    {
        ...
        $row = $order->getInfo(['order_no'=>$order_no,'order_status'=>10,'pay_status'=>0],'order_id,user_name,receiver_mobile,order_no');
        if(!empty($row)){
            $user = new user();
    ...
    public function orderDataExcel()
    {
        ...
        foreach ($list as $k => $v) {
            ...
            if ($v['pay_status'] == 0) {
                ...
                $puser = $user->query($sql);
                $list['data'][$k]['puser'] = $puser[0];
                $list[$k]["order_personnel"] = '上级推广员:'.$puser[0]['nick_name'].'<br/>'.'下单人员:'.$v['nick_name'];
            ...
            $goods_info = "";
            foreach ($v["order_item_list"] as $t => $m) {
                $goods_info .= "课程名称:" . $m["goods_name"] . "  规格:" . $m["sku_name"] . "  课程价格:" . $m["price"] . " ";
            }
            $list[$k]["goods_info"] = $goods_info;
        }
        dataExcel($xlsName, $xlsCell, $list);

...
use data\model\NsMemberAccountModel as memberAccount;
...
class Order extends BaseController
{
    ...
    public function collectMoneyList()
    {
        if (request()->isAjax()) {
            ...
            foreach($result['data'] as $k=>$v){
                $goods_name = explode(',',$v['goods_name']);
                $v['goods_name'] = '';
                foreach($goods_name as $g){
                    $v['goods_name'] .= $g; 
                }
    ...
    public function getOrderNo_user($order_no='')
    {
        ...
        $row = $order->getInfo(['order_no'=>$order_no,'order_status'=>10,'pay_status'=>['not in','-10,-11']],'order_id,user_name,receiver_mobile,order_no,buyer_id');

        if(!empty($row)){
            $user = new user();
            $goods = $OrderGoods->getInfo(['order_id'=>$row['order_id'],'refund_status'=>['not in',5]],'goods_id,goods_name');
            if(!empty($goods)){
                $result['goods_name'] = $goods['goods_name'];
                $result['goods_id'] = $goods['goods_id']; 
            }
            $balance = $memberAccount->getInfo(['uid'=>$row['buyer_id']],'balance');
            $result['goods'] = $goods;
            $result['buyer_id'] = $row['buyer_id'];
            ...
            $Level = $member->getInfo(['uid'=>$row['buyer_id']],'member_level');
    ...
    public function orderList()
    {
        if (request()->isAjax()) {
            ...
            if ($order_status==-12) {
                $condition['pay_status'] = ['in',0,-11,-12,-13]; 
            }elseif ($order_status==-14) {
                $condition['pay_status'] = ['in',-12]; 
            }else{
                if(!empty($order_status)){
                    $condition['pay_status'] = $order_status;
                }
            ...
            foreach($list['data'] as $k=>$v){
                if($v['order_status']==10){
                    ...
                    $sql = "SELECT a.uid,a.nick_name FROM sys_user as a INNER JOIN sys_user as b ON a.uid=b.pid where b.uid=".$v['buyer_id']." limit 0,1"; 
                    $puser = $user->query($sql);
                    if(!empty($puser)){
                       $list['data'][$k]['pnick_name'] = $puser[0]['nick_name']; 
                   }else{
                       $list['data'][$k]['pnick_name'] = '';
                   }
            ...
            $child_menu_list[] = array(
                    'url' => "order/orderlist?status=10",
                    'menu_name' => '已完成',
                    "active" => $status == 10 ? 1 : 0
                );
            $child_menu_list[] = array(
                    'url' => "order/orderlist?status=-12",
                    'menu_name' => '未完成',
                    "active" => $status == -12 ? 1 : 0
                );
            $child_menu_list[] = array(
                    'url' => "order/orderlist?status=-11",
                    'menu_name' => '取消订单中',
                    "active" => $status == -11 ? 1 : 0
                );
            $child_menu_list[] = array(
                    'url' => "order/orderlist?status=-10",
                    'menu_name' => '已关闭',
    ...
    public function orderDataExcel()
    {
        ...
        foreach ($list as $k => $v) {
            ...
            foreach ($v["order_item_list"] as $t => $m) {
                $goods_info .= "课程名称:" . $m["goods_name"] . "  课程价格:" . $m["price"] . " ";
            }
    ...
    public function virtualOrderDataExcel()
    {
        ...
        foreach ($list as $k => $v) {
            ...
            $goods_info = "";
            foreach ($v["order_item_list"] as $t => $m) {
                $goods_info .= "课程名称:" . $m["goods_name"] . "  规格:" . $m["sku_name"] . "  课程价格:" . $m["price"] . "  购买数量:" . $m["num"] . "  ";
            }
            $list[$k]["goods_info"] = $goods_info;
        }
        dataExcel($xlsName, $xlsCell, $list);

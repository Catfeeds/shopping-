...
class Config extends BaseController
{
    ...
    public function webConfig()
    {
        if (request()->isPost()) {
            ...
            $content = request()->post("content", ""); // 关于我们
            
            $retval = $this->website->updateWebSite($title, $logo, $web_desc, $key_words, $web_icp, $web_style_pc, $web_qrcode, $web_url, $web_phone, $web_email, $web_qq, $web_weixin, $web_address, $third_count, $web_popup_title, $web_wechat_share_logo, $web_gov_record, $web_gov_record_url,$content);
    ...
    public function registerAndVisit()
    {
        ...
        if (request()->isAjax()) {
            ...
            $admin_id = $_SESSION['niu']['adminsiteuid'];
            if(CONFIG_YYA==1&&$admin_id==ADMINID||CONFIG_YYA==2){
                $yshow = 1;
            }else{
                $yshow = 2;
            }
            $this->assign('yshow', $yshow);
    ...
    public function pictureUploadSetting()
    {
        ...
        if (request()->isAjax()) {
            ...
            $admin_id = $_SESSION['niu']['adminsiteuid'];
            if(CONFIG_YYC==1&&$admin_id==ADMINID||CONFIG_YYC==2){
                $yshow = 1;
            }else{
                $yshow = 2;
            }
    ...
    public function visitConfig()
    {
        if (request()->isAjax()) {
            ...
            $admin_id = $_SESSION['niu']['adminsiteuid'];
            if(CONFIG_YYB==1&&$admin_id==ADMINID||CONFIG_YYB==2){
                $yshow = 1;
            }else{
                $yshow = 2;
            }

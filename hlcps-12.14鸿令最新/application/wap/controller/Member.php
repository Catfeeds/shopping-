...
use data\model\NsMemberModel as MemberModel;
...
use data\model\NsMemberLevelModel as memberLevel;
...
class Member extends BaseController
{
    ...
    public function memberIndex()
    {
        ...
        $member_menu_arr = array(
            ...
            'qr_code' => array(
                lang('extend_qrcode'),
                'member/getWchatQrcode&uid='.$this->uid
            ),
    ...
    //会员等级说明
    public function degreeStatements(){
        $memberLevel = new memberLevel();
        $member = new MemberModel();
        $levelList = $memberLevel->getQuery([],'*','level_id');
        $member_level = $member->getInfo(['uid'=>$this->uid],'member_level');
        $this->assign("member_level", $member_level);
        $this->assign("levelList", $levelList);
        return view($this->style.'Member/degreeStatements');
    }
    //帮助中心
    ...
    public function getWchatQrcode()
    {
        ...
        $imgsrc = $this->showUserQrcode($uid);
        // 获取微信配置
        $config = new Config();
    ...
    function showUserQrcode($uid)
    {
        ...
        $ppath = $upload_path . '/qrcode_create' . $uid . '_' . $instance_id . '.png';
        if (! file_exists($path)) {
            /*$weixin = new Weixin();
            ...
            $url = "http://bm.hlyjbz.com/index.php?s=/wap/login/code_register&pid=".$uid;
            //$url = "http://bm.hlyjbz.com/index.php?s=/wap/member/index/pid/".$uid;
            getQRcode($url, $upload_path, "qrcode_" . $uid . '_' . $instance_id);
        }
        // 定义中继二维码地址

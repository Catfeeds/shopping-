...
class Goods extends BaseController
{
    public function __destruct(){
        $this->redirect(__URL("/wap/member/index"));
    }
    //资讯列表
    ...
    public function getCartInfo($goods_id)
    {
        ...
        $this->assign("carcount", count($cartlist)); // 购物车课程数量
        $this->assign("num", $num); // 购物车已购买课程数量
    }
    ...
    public function goodsList()
    {
        ...
        if (request()->isAjax()) {
            $category_id = request()->post('category_id', ''); // 课程分类
            $brand_id = request()->post('brand_id', ''); // 品牌
            $order = request()->post('obyzd', ''); // 课程排序分类,order by ziduan
            $sort = request()->post('st', 'desc'); // 课程排序分类 sort
            $page = request()->post('page', 1);

    ...
    public function init()
    {
        ...
        if (! request()->isAjax()) {
         ...
         if (empty($custom_service)) {
            ...
            $this->assign('goods_category_one', $goods_category_tree); // 课程分类一级
            ...
            $this->assign('is_head_goods_nav', 0); // 课程分类是否显示样式
            
            $defaultImages = $config->getDefaultImages($this->instance_id);
            $this->assign("default_goods_img", $defaultImages["value"]["default_goods_img"]);//默认课程图片

 ...
 * 会员控制器
 */
class Member111 extends BaseController
{
    ...
    public function buyNowSession()
    {
        ...
        $cart_list['is_open_presell'] = $goods_info['is_open_presell'];//是否为预售课程
    ...
    {
        // 加入购物车
        $cart_list = isset($_SESSION["cart_list"]) ? $_SESSION["cart_list"] : ""; // 用户所选择的课程
        if (empty($cart_list)) {
            $redirect = __URL(__URL__ . "/index");
            $this->redirect($redirect);
        }
    ...
    public function comboPackageorderInfo()
    {
        ...
        $count_money = $order->getComboPackageGoodsSkuListPrice($goods_sku_list); // 课程金额仅作显示
    ...
    public function orderCreateSession()
    {
        ...
        switch ($tag) {
            case 'buy_now':
                ...
                $_SESSION['order_goods_type'] = request()->post("goods_type", 1); // 课程类型标识
                break;
            case 'cart':
    ...
    public function groupBuyOrderInfo()
    {
        ...
        $count_money = $group_buy_order->getGoodsSkuGroupBuyPrice($goods_sku_list); // 课程金额
...
//         $cost_price = $order->getGoodsSkuListPrice($goods_sku_list); // 课程金额

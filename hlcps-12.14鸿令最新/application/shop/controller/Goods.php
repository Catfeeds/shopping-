...
class Goods extends BaseController
{
    ...
    public function goodsinfo($goodsid)
    {
        ...
        if (! empty($goodsid)) {
            ...
            $this->assign('goods_id', $goodsid); // 将课程id传入方便查询当前课程的评论
            ...
            // 当前用户是否收藏了该课程,uid是从baseController获取到的
    ...
    {
        $category_id = request()->get('category_id', ''); // 课程分类
        ...
        $province_id = request()->get('province_id', ''); // 课程所在地
        ...
        // -----------------查询条件筛选---------------------
        $this->assign("category_id", $category_id); // 课程分类ID
       ...
       // $this->assign("user_location", get_city_by_ip()); // 获取用户位置信息
        ...
        $this->assign("goods_list", $goods_list); // 返回课程列表
        $category_name = "";
        if (! $category_id == "") {
    ...
    /**
     * 根据条件查询课程列表：课程分类查询，关键词查询，价格区间查询，品牌查询
     */
    ...
    public function goodsConsult()
    {
        ...
        $assign_get_list = array(
            'goods_info' => $goods_info, // 课程详情
            'goodsid' => $goodsid, // 课程id
            'page' => $page, // 当前页

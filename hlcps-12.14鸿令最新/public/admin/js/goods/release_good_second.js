	...
	if(parseInt($("#goodsId").val()) > 0){
		...
		if(parseInt($("#goodsType").val()) > 0){
			...
			getGoodsSpecListByAttrId($("#goodsType").val(),function(){
				//现在取消课程属性和课程规格的guanxi
				//加载属性
				$(".js-goods-sku-attribute tr").each(function(){
					
					var value = $(this).children("td:first").attr("data-value");//课程属性名称
...
function ValidateUserInput() {
	...
	$(".ladder_preference").each(function(){
		...
		if(preference >= min_price){
			$(".goods-nav ul li:eq(2)").click();
			showTip("优惠价格不能大于或等于课程最小价格","error");
...
function SubmitProductInfo(type, ADMIN_MAIN,SHOP_MAIN) {//alert("1111");
	...
	if (validateResult) {
		var upload_img_id=$(".upload_img_id").val();
		if(!upload_img_id){
	         layer.msg("请等待图片上传完成");
	         return false;
	   }
		...
		$.ajax({
			...
			success : function(res) {
				...
				if (res != null) {
					...
					showMessage('success', "课程发布成功",__URL(ADMIN_MAIN +'/goods/goodslist'));
				} else {
...
function PackageProductInfo() {
	...
	productViewObj.goods_unit = $("#goodsUnit").val().replace(/^\s*/g, "").replace(/\s*$/g, "");// 课程单位
	...
	productViewObj.code = $("#txtProductCodeA").val();// 课程编码
	...
	productViewObj.province_id = $("#provinceSelect").val();// 课程所在地：省
	productViewObj.city_id = $("#citySelect").val();// 课程所在地：市
	...
	//虚拟课程类型数据
	var virtual_goods_type_data = {};//虚拟课程类型数据

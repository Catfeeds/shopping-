...
$(function() {
	...
	$(".num").blur("input propertychange", function() {
		...
		if(min_buy != 0 && min_buy>num){
			showBox("该课程最少购买"+min_buy+"件","warning");
			$cart.val(min_buy);
			return;
		}else if (num == 0||num<0) {
			$cart.val(1);
...
//		if (money != 0) {
			...
			if(getHeavyArray(shop_arr).length>1){
				...
				$.ajax({
					...
					success : function(res){
						window.location.href = __URL(APPMAIN+"/order/reservationOrder");
					}
				});
			}
//		}
		...
		$(".cart-list-li").each(function() {
			...
			if (is_check == 'yes') {
				...
				if ($(".cart-prolist-ul li[data-shopid='" + shopid+ "']").length == 0) {
					// alert("我这家店的课程都没了，还不快删除我"+shopid);
...
var Cart = {
	...
	changeBar : function(type, skuId, obj,goods_id) {
		...
		if (max_buy != 0 && num > max_buy) {
			showBox("该课程每人限购" + max_buy + "件","warning");
			return;
		}

...
$(function() {
			...
			if ($("#hiddStock").val() == 0) {
				...
				if ($uiskupropCount === flag) {
					...
					if(currentNum!=0 && currentNum == purchaseSum){
						showBox("此课程限购，您最多可购买"+ purchaseSum+ goods_unit,"warning");
						return;
					}
					if (num >= 1) {
						if (num <= nummax) {
										...
										success : function(data) {
											...
											if(data.code>0){
												...
												if ($("#submit_ok").attr("tag") == "addCart") {
													...
													$(".js-shipping-fee-name").html('<a class="btn" href="http://localhost/hlcps/index.php?s=/wap/goods/cart" style="color: #fff;font-size: 12px;border-radius: 2px;width:55%;height:30px;line-height:30px;display:inline-block;background:#ff9800;text-align:center;">查看购物车</a>');
												}
												$("#loading").hide();
											}else if(data.code == -1){
						...
						showBox("课程的数量至少为1","warning");
					}
				} else {
					showBox("请选择完整的课程规格","warning");
				}
			}
		}
	});
	...
	$(".add").click(function() {
		...
		if (num >= max_buy && max_buy != 0) {
			...
			showBox("此课程限购，您最多可购买" + buy_num + goods_unit,"warning");
		} else if (num < nummax) {
			num = num + 1;
	...
	$(".reduce").click(function() {
		...
		// if(count){
		// 	showBox("此课程最少购买" + count + goods_unit);
		// }
		if (num > count) {
			num -= 1;
			if (num == 1) {
	...
	$("#num").bind("input propertychange", function() {
		if($(this).val().indexOf(".") != -1){
			...
			if(min_buy !=0 && min_buy>num){
				showBox("此课程最少购买" + min_buy + goods_unit,"warning");
				num = min_buy;
			}
			if (num >= max_buy && max_buy != 0) {
				showBox("此课程限购，您最多可购买" + max_buy + goods_unit,"warning");
				num = max_buy;
			} else if (num > nummax) {
				num = nummax;
			}

...
var cart_id_arr = new Array();//购物车中的课程Id array
var cart_num = new Array(); //课程的数量 array
...
function validationInfo(goodsid,flag){
	...
	for(var i=0;i<cart_id_arr.length;i++){
		...
		if(cart_id_arr[i]==goodsid && cart_num[i]>1 && cart_num[i]==$("#hidden_max_buy").val()){
			if(flag == "to_cart") $.msg("该课程每人限购"+cart_num[i]+"件");
			else $.msg("该课程每人限购"+cart_num[i]+"件，购物车中已存在该课程");
			return false;
		}
	}
	//再次检查购物车中的课程，是否有同一件课程，不同的SKU
	if(num>0 && num == $("#hidden_max_buy").val()){
		$.msg("购物车中已存在该课程");
		return false;
	}
	//如果该课程是积分课程，则判断当前用户的积分是否足够
...
//删除购物车中的课程  flag:是否刷新当前页面，
function deleteShoppingCartById(id,flag){
	if (confirm("您确实要把该课程移出购物车吗？")){
		$.ajax({
	...
	$.ajax({
		...
		success : function(data){
			...
			if(data.length>0){//没登录会返回首页html代码
				...
				$(".header-right .cart-box-mask-layer .cart-box-bottom .total").html("共"+data.length+"种课程，总金额"+total+"元");
			}else{
				//再次查询
				str += '<p class="cart_empty">您的购物车中暂无课程</p>';

...
$(".js-clear-cart").click(function() {
	var sel_goods = new Array();// 保存选中要购买的课程
	var obj_cartgoods = document.getElementsByName("sel_cartgoods[]");
	var j = 0;
...
function select_cart_goods() {
	...
	var obj_cartgoods = document.getElementsByName("sel_cartgoods[]");
	var j = 0;
	var c = true;// 是否全选要购买的课程：true:全选，false:反之
	for (i = 0; i < obj_cartgoods.length; i++) {
...
function getCopeSum() {
	...
	sum=sum.toFixed(2);
	if (j == 0) {
		$("#cart_money_info").html("您一个课程都没选，这怎么行捏！！真的不行哦！");
...
// 添加数量，需要判断当前购物车中的相同课程数量与限购数量，其次再判断课程的限购数量
/**
 * cart_id :购物车Id max_buy ：限购数量 0 不限购 goodsid ： 课程ID obj : 当前对象 stock ：
 * 对应课程的sku库存
 */
function add_num(cart_id, max_buy, goodsid, obj, stock) {
	...
	if (max_buy == 0) {// 不限购
		...
		getGoodsPurchaseRestrictionForCurrentUser(goodsid,purchase_restriction,function(purchase){
			if(purchase.code == 0){
				temp_num = temp_num-purchase.value;//当前课程数量 - 还能购买的课程数量 = 可购买的课程数量
	...
	var price = $("#goods_price_" + cart_id).text().replace("￥","");// 课程单价
	if (is_update) {
		...
		getCopeSum();// 刷新应付总额
		updateGoodsNumber(cart_id, temp_num,goods_obj);// 更新课程数量
	}
}
...
function minus_num(cart_id, max_buy, stock, min_buy, goodsid, obj) {
	...
	if (number > 1) {
		...
		$("#subtotal_" + cart_id).attr("data-total", total_price);
		getCopeSum();// 刷新应付总额
		updateGoodsNumber(cart_id, number,obj);// 更新课程数量	
	}
}
...
function change_price(cart_id, max_buy, goodsid, obj, stock, min) {
	...
	if (!r.test(number)) {
		...
		if (max_buy == 0) {// 不限购
			...
			getGoodsPurchaseRestrictionForCurrentUser(goodsid,purchase_restriction,function(purchase){
				if(purchase.code == 0){
					temp_num = temp_num-purchase.value;//当前课程数量 - 已购买的课程数量
					$.msg(purchase.message);
					is_update = false;
				}
			});
		...
		if (is_update) {
			...
			getCopeSum();// 刷新应付总额
			updateGoodsNumber(cart_id, temp_num,$(obj));// 更新课程数量
		}
	}
}

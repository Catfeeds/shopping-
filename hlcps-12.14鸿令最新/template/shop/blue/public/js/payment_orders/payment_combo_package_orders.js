...
$(function(){
	...
	$(".btn-jiesuan").click(function(){
		if(validationOrder()){
			...
			flag = true;
			var goods_sku_list = $("#goods_sku_list").val();// 课程Skulist
...
function init(){
	...
	$(".js-goods-num").text($(".goodinfo").length);//课程数量
	...
	//初始化合计
	var init_total_money = parseFloat($("#hidden_combo_package_price").val());//课程金额
	//有运费
...
function calculateTotalAmount(){
	var goods_money = parseFloat($("#hidden_count_money").val()); //课程金额
	...
	var tax_sum = parseFloat($("#hidden_combo_package_price").val());//计算发票税额计算：（课程总计+运-优惠活动-优惠券）*发票税率
	var express = 0; //运费
	//课程金额
	if(goods_money > 0){
	...
	if(getPickupId()>0){
		...
		//满额包邮
		var init_total_money = parseFloat($("#hidden_count_money").val());//课程金额

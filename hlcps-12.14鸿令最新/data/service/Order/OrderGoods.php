...
class OrderGoods extends BaseService
{
     ...
     * goods_id int(11) NOT NULL COMMENT '课程ID',
     ...
     * price decimal(19, 2) NOT NULL DEFAULT 0.00 COMMENT '课程价格',
     * num varchar(255) NOT NULL DEFAULT '0' COMMENT '购买数量',
     * adjust_money varchar(255) NOT NULL DEFAULT '0' COMMENT '调整金额',
     * goods_money varchar(255) NOT NULL DEFAULT '0' COMMENT '课程总价',
     ...
     * goods_type varchar(255) NOT NULL DEFAULT '1' COMMENT '课程类型',
        ...
        try {
            ...
            foreach ($goods_sku_list_array as $k => $goods_sku_array) {
                ...
                $data_order_sku = array(
                    ...
                    'goods_picture' => $picture != 0 ? $picture : $goods_info['picture'], // 如果当前课程有SKU图片，就用SKU图片。没有则用课程主图
        ...
        try {
            ...
            foreach ($goods_sku_list_array as $k => $goods_sku_array) {
                ...
                $data_order_sku = array(
                    ...
                    'cost_price' => 0,
                    'goods_money' => 0,
                    'goods_picture' => $goods_info['picture'], // 如果当前课程有SKU图片，就用SKU图片。没有则用课程主图
                    'shop_id' => $this->instance_id,
                    'buyer_id' => $this->uid,
                    'goods_type' => $goods_info['goods_type'],
                    'promotion_id' => $goods_info['promote_id'],
     ...
     * 根据课程规格信息查询SKU主图片
     *
     * @param 课程规格信息 $goods_sku_info            
     * @return 0：没有查询到课程SKU图片，!0:查询到了课程SKU图片
     */
    ...
    public function orderGoodsRefundAgree($order_id, $order_goods_id)
    {
        ...
        try {
            ...
            if($orders['order_status']==10){
                $count = $order_goods->getCount(['order_id'=>$order_id]);
                $count2 = $order_goods->getCount(['order_id'=>$order_id,'refund_status'=>1]);
                $count3 = $order_goods->getCount(['order_id'=>$order_id,'refund_status'=>-1]);
                $count4 = $order_goods->getCount(['order_id'=>$order_id,'refund_status'=>5]);
                if($count==0||$count==$count4){
                    $order_model->ModifyTableField('order_id',$order_id,'pay_status','-10');
                }elseif($count2>0){
                    $order_model->ModifyTableField('order_id',$order_id,'pay_status','-11');
                }elseif($count==$count3){
                    $order_model->ModifyTableField('order_id',$order_id,'pay_status','-13');
                }else{
                    $order_model->ModifyTableField('order_id',$order_id,'pay_status','-12');
                }
            }
            $this->order_goods->commit();
            return 1;

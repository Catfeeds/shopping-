...
class Order extends BaseService
{
    ...
    public function orderCreate($order_type, $out_trade_no, $pay_type, $shipping_type, $order_from, $buyer_ip, $buyer_message, $buyer_invoice, $shipping_time, $receiver_mobile, $receiver_province, $receiver_city, $receiver_district, $receiver_address, $receiver_zip, $receiver_name, $point, $coupon_id, $user_money, $goods_sku_list, $platform_money, $pick_up_id, $shipping_company_id, $coin, $fixed_telephone = "", $presell_money = 0)
    {
        ...
        try {
            ...
            $data_order = array(
                ...
                'goods_money' => $goods_money, // decimal(19, 2) NOT NULL COMMENT '课程总价',
     ...
     * @param unknown $memo
     */
    public function orderCreateReservation($out_trade_no, $goods_sku_list, $coupon_id, $receiver_name, $receiver_mobile, $buyer_message)
    {
        $this->order->startTrans();
        ...
        try {
            ...
            $data_order = array(
                ...
                'buyer_message' => $buyer_message,
                'buyer_invoice' => $buyer_invoice,
                'receiver_name' => $receiver_name, // datetime NOT NULL COMMENT '买家要求配送时间',
                'receiver_mobile' => $receiver_mobile, // varchar(11) NOT NULL DEFAULT '' COMMENT '收货人的手机号码',
                ...
                'shop_id' => $shop_id, // int(11) NOT NULL COMMENT '卖家店铺id',
                'shop_name' => $shop_name, // varchar(100) NOT NULL DEFAULT '' COMMENT '卖家店铺名称',
                'goods_money' => 0, // decimal(19, 2) NOT NULL COMMENT '课程总价',
            ...
            if ($coupon_id[0] > 0) {
                ...
                $i=0;
                foreach($coupon_id as $c){
                    $max_fetch = $CouponType->getInfo(['coupon_type_id'=>$c],'max_fetch');
                    $couponCount = $Coupon->getCount(['coupon_type_id'=>$c,'state'=>['gt','0'],'uid'=>$this->uid]);
                    if($max_fetch>0&&$couponCount<$max_fetch||$max_fetch==0){
                        ...
                        $couponid = $Coupon->getInfo(['coupon_type_id'=>$c,'state'=>0],'coupon_id');
                        $Coupon_state = $Coupon->where('coupon_id',$couponid['coupon_id'])->update($data_details);
                        $i++;
                    }
                }
            }
            // 添加订单项
            $order_goods = new OrderGoods();
            $res_order_goods = $order_goods->addOrderGoodsReservation($order_id, $goods_sku_list,0);
    ...
    private function calculateOrderGivePoint($order_id)
    {
        ...
        $member_account->addMemberAccountData($point['shop_id'], 1, $point['buyer_id'], 1, $point['give_point'], 1, $order_id, '订单课程赠送积分');
    }
    ...
    public function addPromotionGiftGrantRecords($order_id, $uid, $nick_name)
    {
        ...
        $gift_goods_model = new NsPromotionGiftGoodsModel(); // 课程赠品
        $promotion = new Promotion();

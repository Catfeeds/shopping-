...
class OrderAccount extends BaseService
{
     ...
     * @param unknown $end_time
     */
    public function getShopOrderSumCancel($shop_id, $start_time, $end_time)
    {
        $order_model = new NsOrderModel();
        $condition["create_time"] = [
            [
        ...
        $order_sum = $order_model->getCount($condition, 'order_id');
        return $order_sum;
    ...
    public function getShopSaleGoodsNumSum($condition){
        ...
        if($order_string != ''){
            ...
            $goods_num = $order_goods_model->getCount($condition,"order_id");
        }
        if(!empty($goods_num))
        {
            return $goods_num;

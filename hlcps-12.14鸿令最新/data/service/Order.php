...
class Order extends BaseService implements IOrder
{
    ...
    public function updateCollectMoney($collect_id, $order_id, $goods_id, $goods_name, $reserve_phone, $order_no, $uid, $nick_name, $money, $meet_money, $coupon_name, $coupon_id, $coupon_money, $at_leasts, $realPayment_money, $payment_method, $accountsWx_money, $paymentDate, $remark, $create_time)
    {
        ...
        $orderSum = $order_model->getSum(['buyer_id'=>$uid,'pay_status'=>10],'pay_money');
        ...
        $level2 = $member_level->getInfo(['level_id'=>$memberLevel['member_level']],'quota');
        //修改等级
        $orderSum = $order_model->getSum(['buyer_id'=>$uid,'pay_status'=>10],'pay_money');
        $total_money = $orderSum+$meet_money;
        if($level2['quota']<$total_money){ //如果当前等级大于要修改的等级就不执行
            $level = $member_level->getMax(['quota'=>['elt',$total_money]],'level_id');
            $member->ModifyTableField('uid',$uid,'member_level',$level);
        }
        //发放线下订单佣金
        $result5 = true;
        $result6 = true;
     ...
     */
    public function orderCreateReservation($out_trade_no, $goods_sku_list, $use_coupon, $receiver_name, $receiver_mobile, $buyer_message)
    {
        $order = new OrderBusiness();
        $retval = $order->orderCreateReservation($out_trade_no, $goods_sku_list, $use_coupon, $receiver_name, $receiver_mobile, $buyer_message);
        runhook("Notify", "orderCreate", array(
            "order_id" => $retval
        ));
        // 针对特殊订单执行支付处理
            ...
            if(!empty($orderList)){
                foreach($orderList as $k=>$v){
                    ...
                    if($goodsCon['is_open']==1){
                        ...
                        $relationList = $Relations->getQuery(['ChildId'=>$v['buyer_id']],'Pid','FxLevel');
                        $j = 1;
                        if(!empty($relationList)){
                            foreach($relationList as $p){
                                if(LEVEL>$j){
                                    $is_promoters = $user->getInfo(['uid'=>$p['Pid']],'is_promoters,level_id');
    ...
    /**
     * 查询订单中的课程是否有限购，如果有限购，则查询是否有购买记录等
     */
        ...
        if (count($res_array) > 0) {
            foreach ($res_array as $k => $v) {
                if ($res_array[$k]['code'] == 0) {
                    $messages = "您购买的课程“" . $res_array[$k]['goods_name'] . "”限购" . $res_array[$k]['value'] . "件";
                    break;
                }
            }
        }
    ...
    public function getGoodsStock($goods_sku_list)
    {
        ...
        if ($goods['stock'] <= 0) {
            $messages = "您购买的课程<<" . $goods['goods_name'] . ">>已经没有名额了";
        }
        return $messages;
    }
    ...
    /**
     * 删除购物车中的数据
     * 首先要查询当前课程在购物车中的数量，如果课程数量等于1则删除，如果课程数量大于1个，则减少该课程的数量
     * (non-PHPdoc)
     *
    ...
    { 
        ...
        $order_refund_sum = $order_account->getShopOrderSumCancel($shop_id, $start_time, $end_time);
        $order_sum_account = $order_sum;
        $array = array(
            'order_sum' => $order_sum,
